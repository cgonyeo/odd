...
void dammitAnimation(double *params, double totalTime, odd_led_t *color, double *storage)
{
    ...
    for(int i = 0; i < NUM_LEDS / 2; i++)
    {
        ...
        ledSums[i] *= (logf(i + 15) / logf(NUM_LEDS + 15)) * 66;
    }

    if(color->next == NULL)
        for(int i = 0; i < NUM_LEDS / 2; i++)
        {
            setTempLED(NUM_LEDS / 2 - i - 1, 'r', ledSums[i] * color->R);
            setTempLED(NUM_LEDS / 2 - i - 1, 'g', ledSums[i] * color->G);
            setTempLED(NUM_LEDS / 2 - i - 1, 'b', ledSums[i] * color->B);
            setTempLED(NUM_LEDS / 2 + i,     'r', ledSums[i] * color->R);
            setTempLED(NUM_LEDS / 2 + i,     'g', ledSums[i] * color->G);
            setTempLED(NUM_LEDS / 2 + i,     'b', ledSums[i] * color->B);
        }
    else
        for(int i = 0; i < NUM_LEDS / 2; i++)
        {
            ...
            setTempLED(NUM_LEDS / 2 - i - 1, 'r', ledSums[i] * redChannel);
            setTempLED(NUM_LEDS / 2 - i - 1, 'g', ledSums[i] * greenChannel);
            setTempLED(NUM_LEDS / 2 - i - 1, 'b', ledSums[i] * blueChannel);
